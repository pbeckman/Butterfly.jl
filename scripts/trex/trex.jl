...
mesh_file     = ""
tree_file     = "trex-tree.csv"
block_file    = "trex-blocks.csv"
file_template = "output/trex_eigenfunctions_n437504_b100"
bs            = 0:30
files         = [file_template * "_k$k.csv" for k=bs]
dedup_files   = [file_template * "_k$(k)_dedup.bin" for k=bs]

xs           = Matrix(readdlm(mesh_file, ',', Float64)')
keep_patches = vec(readdlm(keep_file, ',', Int64))
...
function split_conjugates(M)
    conj_inds = findall(abs.(imag.(M[1,:])) .> 0)
